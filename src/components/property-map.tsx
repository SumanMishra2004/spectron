...
if (typeof window !== 'undefined') {
  try {
    ...
    console.warn('Marker clustering not available');
  }
}
...
function MapUpdater({ center, selectedProperty }: { 
  center: [number, number]; 
  selectedProperty: Property | null;
}) {
  const map = useMap();
...
export default function PropertyMap({
  ...
  useEffect(() => {
    ...
    const markerClusterGroup = MarkerClusterGroup ? new MarkerClusterGroup({
      maxClusterRadius: 50,
      ...
      iconCreateFunction: (cluster: any) => {
        ...
        return L.divIcon({
          html: `<div class="bg-spectron-teal text-white rounded-full w-10 h-10 flex items-center justify-center font-bold shadow-lg">${count}</div>`,
    ...
    properties.forEach((property) => {
      ...
      const popupContent = `
        <div class="p-2 min-w-[200px]">
          ...
          <p class="text-xs text-gray-600 mb-2">${property.bhk} BHK • ${property.area} sq ft</p>
          <p class="text-sm font-semibold text-spectron-teal">
  ...
  return (
    ...
    >
      ...
      <MapUpdater center={center} selectedProperty={selectedProperty} />
    </MapContainer>
  );
}
